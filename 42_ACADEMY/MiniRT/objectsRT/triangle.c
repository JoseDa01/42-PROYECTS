...
/*   Updated: 2021/03/29 20:16:30 by jtello-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_intersection	*trianglecollision(t_rayo ray, t_triangle *tri)
{
	...
	solution = vectorialspacesolution(tri->point1,
			tri->vect1, tri->vect2, pointinter);
	if (solution.tam <= 0)
		return (NULL);
	if (!(solution.tupla[0] >= 0 && solution.tupla[1] >= 0
			&& (solution.tupla[0] + solution.tupla[1] <= 1)))
		return (NULL);
